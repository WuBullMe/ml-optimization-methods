...
if __name__ == '__main__':
    ...
    if 'from_path' in config['model']['params']:
        model = BaseModule.load_from_checkpoint(config["model"]["params"]['from_path'], map_location='cpu', strict=False)
    ...
    trainer = pl.Trainer(
         **config["optimization"]["hardware"],
        **config["optimization"]["train"]
    )

    res = trainer.validate(model, dataloaders.get('val', dataloaders['test']))
